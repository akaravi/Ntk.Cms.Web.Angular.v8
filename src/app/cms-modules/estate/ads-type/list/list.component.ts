...
import {
  DataFieldInfoModel,
  ErrorExceptionResult, EstateAdsTypeModel,
  EstateAdsTypeService, FilterDataModel, FilterModel, RecordStatusEnum, SortTypeEnum, TokenInfoModel
} from 'ntk-cms-api';
...
export class EstateAdsTypeListComponent implements OnInit, OnDestroy {
  constructor(
  ...
  expandedElement: EstateAdsTypeModel | null;
  ...
  onActionbuttonContentList(model: EstateAdsTypeModel = this.tableRowSelected, event?: MouseEvent): void {
    ...
    if (event?.ctrlKey) {
      this.link = "/#/estate/property/LinkAdsTypeId/" + this.tableRowSelected.id;
