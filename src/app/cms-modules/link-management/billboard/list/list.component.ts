...
import {
  ...
  FilterDataModel,
  FilterModel,
  LinkManagementBillboardModel, LinkManagementBillboardPatternModel, LinkManagementBillboardService,
  TokenInfoModel
} from 'ntk-cms-api';
...
export class LinkManagementBillboardListComponent implements OnInit, OnDestroy {
  ...
  onActionbuttonDeleteRow(model: LinkManagementBillboardModel = this.tableRowSelected): void {
    ...
    const dialogRef = this.dialog.open(LinkManagementBillboardDeleteComponent, { height: '90%', data: { id: this.tableRowSelected.id } });
