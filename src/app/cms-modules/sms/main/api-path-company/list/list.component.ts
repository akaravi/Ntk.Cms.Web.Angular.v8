...
import {
  DataFieldInfoModel, ErrorExceptionResult, FilterDataModel, FilterModel, RecordStatusEnum, SmsMainApiPathCompanyModel,
  SmsMainApiPathCompanyService, SortTypeEnum, TokenInfoModel
} from 'ntk-cms-api';
...
export class SmsMainApiPathCompanyListComponent implements OnInit, OnDestroy {
  constructor(
    public contentService: SmsMainApiPathCompanyService,
  ...
  DataGetAll(): void {
    ...
    this.onActionTableRowSelect(new SmsMainApiPathCompanyModel());
  ...
  onActionbuttonApiList(model: SmsMainApiPathCompanyModel = this.tableRowSelected): void {
    ...
    this.router.navigate(['/sms/main/api-path/list/LinkCompanyId', this.tableRowSelected.id]);
