...
import {
  ...
  DataProviderPlanSourceService, RecordStatusEnum, SortTypeEnum,
...
export class DataProviderPlanSourceListComponent implements OnInit, OnDestroy {
  ...
  DataGetAll(): void {
    ...
    this.onActionTableRowSelect(new DataProviderPlanSourceModel());
  ...
  onActionbuttonEditRow(model: DataProviderPlanSourceModel = this.tableRowSelected): void {
    ...
    const dialogRef = this.dialog.open(DataProviderPlanSourceEditComponent, dialogConfig);
    dialogRef.afterClosed().subscribe(result => {
      // console.log(`Dialog result: ${result}`);
      if (result && result.dialogChangedDate) {
        this.DataGetAll();
  ...
  onActionbuttonDeleteRow(model: DataProviderPlanSourceModel = this.tableRowSelected): void {
    ...
    const dialogRef = this.dialog.open(DataProviderPlanSourceDeleteComponent, { height: '40%', data: { id: this.tableRowSelected.id } });
    dialogRef.afterClosed().subscribe(result => {
      // console.log(`Dialog result: ${result}`);
      if (result && result.dialogChangedDate) {
        this.DataGetAll();
