...
import {
  ...
  DataProviderPlanClientService, EnumRecordStatus, EnumSortType,
...
export class DataProviderPlanClientListComponent implements OnInit, OnDestroy {
  requestLinkPlanId = 0;
  ...
  onActionbuttonEditRow(model: DataProviderPlanClientModel = this.tableRowSelected): void {
    ...
    const dialogRef = this.dialog.open(DataProviderPlanClientEditComponent, dialogConfig);
    dialogRef.afterClosed().subscribe(result => {
      // console.log(`Dialog result: ${result}`);
      if (result && result.dialogChangedDate) {
        this.DataGetAll();
  ...
  onActionbuttonDeleteRow(model: DataProviderPlanClientModel = this.tableRowSelected): void {
    ...
    const dialogRef = this.dialog.open(DataProviderPlanClientDeleteComponent, { height: '40%', data: { id: this.tableRowSelected.id } });
    dialogRef.afterClosed().subscribe(result => {
      // console.log(`Dialog result: ${result}`);
      if (result && result.dialogChangedDate) {
        this.DataGetAll();
