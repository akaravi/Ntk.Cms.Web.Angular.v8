...
export class MemberPropertyAliasListComponent implements OnInit, OnDestroy {
  constructor(
  ...
  DataGetAll(): void {
    ...
    this.tableRowSelected = new MemberPropertyAliasModel();
  ...
  onActionbuttonDeleteRow(model: MemberPropertyAliasModel = this.tableRowSelected): void {
    ...
    const dialogRef = this.dialog.open(MemberPropertyAliasDeleteComponent, { height: '90%', data: { id: this.tableRowSelected.id } });
    dialogRef.afterClosed().subscribe(result => {
      if (result && result.dialogChangedDate) {
        this.DataGetAll();
      }
