...
import {
  CoreEnumService, CoreModuleLogSiteUserCreditModel, CoreModuleLogSiteUserCreditService, CoreSiteModel, DataFieldInfoModel, ErrorExceptionResult, FilterDataModel, FilterModel, RecordStatusEnum, SortTypeEnum, TokenInfoModel
} from 'ntk-cms-api';
...
export class CoreModuleLogSiteUserCreditListComponent implements OnInit, OnDestroy {
  ...
  DataGetAll(): void {
    ...
    this.onActionTableRowSelect(new CoreModuleLogSiteUserCreditModel());
  ...
  onActionbuttonViewRow(model: CoreModuleLogSiteUserCreditModel = this.tableRowSelected): void {
    ...
    const dialogRef = this.dialog.open(CoreModuleLogSiteUserCreditViewComponent, {
      height: '90%',
  ...
  onActionbuttonEditRow(model: CoreModuleLogSiteUserCreditModel = this.tableRowSelected): void {
    ...
    const dialogRef = this.dialog.open(CoreModuleLogSiteUserCreditEditComponent, {
      height: '90%',
      data: { id: this.tableRowSelected.id }
    });
    dialogRef.afterClosed().subscribe(result => {
