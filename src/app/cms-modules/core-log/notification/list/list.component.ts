...
import {
  CoreEnumService, CoreLogNotificationModel, CoreLogNotificationService, CoreSiteModel, DataFieldInfoModel, ErrorExceptionResult, FilterDataModel, FilterModel, InfoEnumModel, RecordStatusEnum, SortTypeEnum, TokenInfoModel
} from 'ntk-cms-api';
...
import { CoreLogNotificationEditComponent } from '../edit/edit.component';
...
export class CoreLogNotificationListComponent implements OnInit, OnDestroy {
  requestLinkSiteId = 0;
  requestLinkUserId = 0;
  ...
  tableRowsSelected: Array<CoreLogNotificationModel> = [];
  tableRowSelected: CoreLogNotificationModel = new CoreLogNotificationModel();
  tableSource: MatTableDataSource<CoreLogNotificationModel> = new MatTableDataSource<CoreLogNotificationModel>();
  ...
  DataGetAll(): void {
    ...
    this.onActionTableRowSelect(new CoreLogNotificationModel());
  ...
  onActionbuttonViewRow(model: CoreLogNotificationModel = this.tableRowSelected): void {
    ...
    const dialogRef = this.dialog.open(CoreLogNotificationViewComponent, {
      height: '90%',
  ...
  onActionbuttonEditRow(model: CoreLogNotificationModel = this.tableRowSelected): void {
    ...
    const dialogRef = this.dialog.open(CoreLogNotificationEditComponent, {
      height: '90%',
      data: { id: this.tableRowSelected.id }
    });
    dialogRef.afterClosed().subscribe(result => {
  ...
  onActionTableRowSelect(row: CoreLogNotificationModel): void {
