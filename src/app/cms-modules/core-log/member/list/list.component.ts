...
import {
  CoreEnumService, CoreLogMemberModel, CoreLogMemberService, CoreSiteModel, DataFieldInfoModel, ErrorExceptionResult, FilterDataModel, FilterModel, InfoEnumModel, RecordStatusEnum, SortTypeEnum, TokenInfoModel
} from 'ntk-cms-api';
...
import { CoreLogMemberEditComponent } from '../edit/edit.component';
...
export class CoreLogMemberListComponent implements OnInit, OnDestroy {
  requestLinkSiteId = 0;
  requestLinkUserId = 0;
  ...
  tableRowsSelected: Array<CoreLogMemberModel> = [];
  tableRowSelected: CoreLogMemberModel = new CoreLogMemberModel();
  tableSource: MatTableDataSource<CoreLogMemberModel> = new MatTableDataSource<CoreLogMemberModel>();
  ...
  DataGetAll(): void {
    ...
    this.onActionTableRowSelect(new CoreLogMemberModel());
  ...
  onActionbuttonViewRow(model: CoreLogMemberModel = this.tableRowSelected): void {
    ...
    const dialogRef = this.dialog.open(CoreLogMemberViewComponent, {
      height: '90%',
  ...
  onActionbuttonEditRow(model: CoreLogMemberModel = this.tableRowSelected): void {
    ...
    const dialogRef = this.dialog.open(CoreLogMemberEditComponent, {
      height: '90%',
      data: { id: this.tableRowSelected.id }
    });
    dialogRef.afterClosed().subscribe(result => {
