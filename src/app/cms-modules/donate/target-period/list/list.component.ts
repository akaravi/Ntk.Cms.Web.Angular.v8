...
import {
  ...
  DonateTargetPeriodService, EnumRecordStatus, EnumSortType,
...
export class DonateTargetPeriodListComponent implements OnInit, OnDestroy {
  ...
  onActionbuttonDeleteRow(model: DonateTargetPeriodModel = this.tableRowSelected): void {
    ...
    const dialogRef = this.dialog.open(DonateTargetPeriodDeleteComponent, { height: '40%', data: { id: this.tableRowSelected.id } });
    dialogRef.afterClosed().subscribe(result => {
      if (result && result.dialogChangedDate) {
        this.DataGetAll();
      }
