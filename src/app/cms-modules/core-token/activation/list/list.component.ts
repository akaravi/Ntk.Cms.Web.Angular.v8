...
import {
  CoreSiteModel, CoreTokenActivationModel, CoreTokenActivationService, DataFieldInfoModel, ErrorExceptionResult, FilterDataModel, FilterModel, InfoEnumModel, RecordStatusEnum, SortTypeEnum, TokenInfoModel
} from 'ntk-cms-api';
...
export class CoreTokenActivationListComponent implements OnInit, OnDestroy {
  ...
  onActionbuttonViewRow(model: CoreTokenActivationModel = this.tableRowSelected): void {
    ...
    const dialogRef = this.dialog.open(CoreTokenActivationViewComponent, {
      height: '90%',
  ...
  onActionbuttonEditRow(model: CoreTokenActivationModel = this.tableRowSelected): void {
    ...
    const dialogRef = this.dialog.open(CoreTokenActivationEditComponent, {
      height: '90%',
      data: { id: this.tableRowSelected.id }
    });
    dialogRef.afterClosed().subscribe(result => {
