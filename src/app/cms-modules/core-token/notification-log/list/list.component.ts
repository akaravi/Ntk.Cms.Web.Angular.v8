...
import {
  CoreSiteModel, CoreTokenNotificationLogModel, CoreTokenNotificationLogService, DataFieldInfoModel, ErrorExceptionResult, FilterDataModel, FilterModel, InfoEnumModel, RecordStatusEnum, SortTypeEnum, TokenInfoModel
} from 'ntk-cms-api';
...
export class CoreTokenNotificationLogListComponent implements OnInit, OnDestroy {
  requestLinkSiteId = 0;
  requestLinkUserId = 0;
  ...
  DataGetAll(): void {
    ...
    this.onActionTableRowSelect(new CoreTokenNotificationLogModel());
  ...
  onActionbuttonViewRow(model: CoreTokenNotificationLogModel = this.tableRowSelected): void {
    ...
    const dialogRef = this.dialog.open(CoreTokenNotificationLogViewComponent, {
      height: '90%',
  ...
  onActionbuttonEditRow(model: CoreTokenNotificationLogModel = this.tableRowSelected): void {
    ...
    const dialogRef = this.dialog.open(CoreTokenNotificationLogEditComponent, {
      height: '90%',
      data: { id: this.tableRowSelected.id }
    });
    dialogRef.afterClosed().subscribe(result => {
